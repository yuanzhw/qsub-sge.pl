...
=head1 Usage
  ...
  --maxproc <num>   set the maximum number of process in queue, default 80
  ...
  --resource <str>  set the required resource used in qsub -l option, default vf=3.0G
  --independent     max number of process in queue process uncontain other qsub process,default null.
  --Check <yes/no>  check last qsub uncomplete to continue,default yes.
...
=head1 Exmple
  ...
  perl qsub-sge.pl --queue all.q -interval 1 -lines 3 --maxproc 10  ./work.sh
  ...
  perl qsub-sge.pl --queue all.q --resource vf=1.9G --reqsub ./work.sh
...
my ($Queue, $Interval, $Lines, $Maxjob, $Maxproc, $Convert,$Secure,$Reqsub,$Resource,$Independent,$Old,$Verbose, $Help);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"maxproc:i"=>\$Maxproc,
...
$Resource=~m/vf=(.+)G/;
#$Queue="middle.q" if($1>=15);
#$Queue="great.q" if($1>=30);
...
if ($Old eq "no"|| @old_qsub<1) {####for uncomplete shell to continue.by heh 2012-5-20
	...
	for (my $i=0; $i<@Shell; $i++) {
		while (1) {
			...
			if ($i<3 || &queueJob() < $Maxproc) {
				...
				sleep 30 if($Interval<30);
				sleep $Interval if($Interval>=30);
			}
		}
	}
